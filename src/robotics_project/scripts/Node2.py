...
def callback_contour_points(msg):
    global accumulated_points
    accumulated_points.append((msg.point.x, msg.point.y, msg.point.z))

def process_accumulated_points(event):
    global i, accumulated_points
    if accumulated_points:
        sorted_points = sort_points(accumulated_points)
        for j in range(0, len(sorted_points)-1, 2):
            x1, y1, z1 = sorted_points[j]
            x2, y2, z2 = sorted_points[j + 1]
        ...
        for j in range(0, len(sorted_points)-2, 1):
            x1, y1, z1 = sorted_points[j]
            x2, y2, z2 = sorted_points[j+2]
...
def main():
    ...
    # Aggiungi un subscriber per la topic "/camera/contour_points"
    rospy.Subscriber("/camera/contour_points", PointStamped, callback_contour_points)
    rospy.Timer(rospy.Duration(1.0), process_accumulated_points)
    ...
    coordinates = sort_points(accumulated_points)
