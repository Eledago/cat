...
# Definizione delle dimensioni della finestra grafica
window_width = 500
window_height = 300
...
# Callback per la rilevazione del contatto
def contact_callback(data):
    ...
    for contact_state in data.states:
        ...
        if 0.5 < force < 10:
            ...
            show_warning_window()
            rospy.sleep(1)
            # Se si verifica il contatto ritorna alla posa iniziale
            return_to_init_pos()
            rospy.loginfo("Ritorno alla posizione iniziale. Ripianifico...")
            shutdown = False

# Pubblicazione della posizione iniziale
